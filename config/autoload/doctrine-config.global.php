...
unset($doctrineModuleConfig['service_manager']);//destrói a chave não utilizada, e seu arrray
unset($doctrineModuleConfig['dependencies']['factories']['doctrine.cli']);//exclui o cli.config->linha de comando

//criando nova configuração do cli.config
$doctrineModuleConfig['dependencies']['factories']['doctrine.cli'] = function(\Interop\Container\ContainerInterface $container){
    
    $cli = new Application;//aplicação do console do symfony
    ...
    $cli->addCommands(array_map(array($container, 'get'), $commands));//adicionando comandos dentro do console do symfony
...
return ArrayUtils::merge($doctrineModuleConfig, $doctrineOrmModuleConfig);//ArrayUtils - Zendframework 2 - operações com array
